...
void Mpu6050Node::ImuPubCallback()
{
    ...
    /* Set covariance */
    message.linear_acceleration_covariance[0] = 0.1;
    message.linear_acceleration_covariance[4] = 0.1;
    message.linear_acceleration_covariance[8] = 0.1;
    ...
    // 🔥 Chuẩn hóa mapping ROS2:
    message.linear_acceleration.x = -(AccelData.Accel_Y - accel_y_offset_);
    message.linear_acceleration.y =  (AccelData.Accel_X - accel_x_offset_);
    message.linear_acceleration.z = -(AccelData.Accel_Z - accel_z_offset_);

    message.angular_velocity.x =  (GyroData.Gyro_Y - gyro_y_offset_) * (M_PI / 180.0);
    message.angular_velocity.y = -(GyroData.Gyro_X - gyro_x_offset_) * (M_PI / 180.0);
